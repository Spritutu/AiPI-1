...
void CAIPITabView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_GRID_PM, pMainFrame->m_GridPM);
...
BEGIN_MESSAGE_MAP(CAIPITabView, COXSizeDialogBar)
	...
	//GRID_PM	
	ON_NOTIFY(NM_DBLCLK, IDC_GRID_PM, OnGridDblClick)
    ON_NOTIFY(NM_CLICK, IDC_GRID_PM, OnGridClick)
	ON_NOTIFY(NM_RCLICK, IDC_GRID_PM, OnGridRClick)
	ON_NOTIFY(GVN_BEGINLABELEDIT, IDC_GRID_PM, OnGridStartEdit)
	ON_NOTIFY(GVN_ENDLABELEDIT, IDC_GRID_PM, OnGridEndEdit)
	ON_NOTIFY(GVN_SELCHANGING, IDC_GRID_PM, OnGridStartSelChange)
	ON_NOTIFY(GVN_SELCHANGED, IDC_GRID_PM, OnGridEndSelChange)
	ON_NOTIFY(GVN_BEGINDRAG, IDC_GRID_PM, OnGridBeginDrag)
	ON_NOTIFY(NM_RCLICK, IDC_GRID_PM, OnGridRClick)
	ON_NOTIFY(NM_RCLICK, IDC_GRID_PM, OnGridRClick)
	ON_NOTIFY(NM_RCLICK, IDC_GRID_PM, OnGridRClick)
	//GRID_WM	
...
void CAIPITabView::CreateTab()
{
	...
	{
	case LANG_SPANISH:
		pTabCtrlItem.pszText = _T("MP");
		break;
	default:
		pTabCtrlItem.pszText = _T("PM");
		break;
	}
	...
	m_TabCtrl.InsertItem( 3, &pTabCtrlItem );	// third tab
	
	
	switch(dwLanguageID)
	{
		case LANG_SPANISH:
		pTabCtrlItem.pszText = _T("MB");
		break;
		default:
...
BOOL CAIPITabView::TabToolTips(UINT id, NMHDR * pNMHDR, LRESULT * pResult )
{
	...
	if(pTTT->hdr.idFrom == 0)
	{
		...
		{
			case LANG_SPANISH:
			pTTT->lpszText = _T("Memoria de Producciones");
			break;
			default:
			pTTT->lpszText = _T("Production Memory");
			break;
		}
		return TRUE;
	}
	...
	if(pTTT->hdr.idFrom == 2)
	{
		...
		{
			case LANG_SPANISH:
			pTTT->lpszText = _T("Agenda de Reglas");
			break;
			default:
			pTTT->lpszText = _T("Rules Agenda");
			break;
		}
		return TRUE;	
	}
	...
	if(pTTT->hdr.idFrom == 3)
	{
		...
		{
			case LANG_SPANISH:
			pTTT->lpszText = _T("Memoria Alpha");
			break;
			default:
...
BOOL CAIPITabView::OnInitDialog() 
{
	...
	pEditPage->m_pGrid = &pMainFrame->m_GridPM;
	...
	pInsertPage->m_pGrid = &pMainFrame->m_GridPM;
	...
	pFormatPage->m_pGrid = &pMainFrame->m_GridPM;
	...
	pOptionsPage->m_pGrid = &pMainFrame->m_GridPM;
	...
	pCellSizePage->m_pGrid = &pMainFrame->m_GridPM;
...
void CAIPITabView::OnSelchangeAipiTab(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	switch(iCurTab)
	{
		...
		case 0:		// PM
			//Page Production Memory
			pMainFrame->m_CurrentGrid = GRID_PM;
			
			
			pMainFrame->m_GridPM.ShowWindow(SW_SHOW);
			...
			pInsertPage->UpdateEditControls();
			m_stcLabelRange.SetWindowText(_T("Production Memory Range:"));
			UpdateMenuUI();
		
			FillGridPM();

			UpdateData(true);
			m_iMaxRange = m_iMaxRange_PM;
			m_iMinRange = m_iMinRange_PM;
			UpdateData(false);
		...
		case 1:		// WM
			...
			pMainFrame->m_GridPM.ShowWindow(SW_HIDE);
...
void CAIPITabView::Init_Page()
{
	...
	//GridCtrl PM
	m_LayoutManager.SetConstraint(IDC_GRID_PM, OX_LMS_TOP, OX_LMT_OPPOSITE, 5, IDC_STC_AIPI_DATA);
	m_LayoutManager.SetConstraint(IDC_GRID_PM, OX_LMS_BOTTOM, OX_LMT_SAME, -5);
	m_LayoutManager.SetConstraint(IDC_GRID_PM, OX_LMS_LEFT, OX_LMT_OPPOSITE, 5, IDC_ROLLUPCTRL);
	m_LayoutManager.SetConstraint(IDC_GRID_PM, OX_LMS_RIGHT, OX_LMT_SAME, -5);
...
void CAIPITabView::InitGridPM(int n_row, int n_col)
{
	    ...
	    pMainFrame->m_GridPM.SetEditable(pMainFrame->m_bEditable);
        pMainFrame->m_GridPM.SetAutoSizeStyle();
		pMainFrame->m_GridPM.EnableDragAndDrop(TRUE);
		pMainFrame->m_GridPM.GetDefaultCell(FALSE, FALSE)->SetBackClr(RGB(0xFF, 0xFF, 0xE0));
		pMainFrame->m_GridPM.SetFixedColumnSelection(TRUE);
		pMainFrame->m_GridPM.SetFixedRowSelection(TRUE);
	    


	    TRY 
		{
		    pMainFrame->m_GridPM.SetRowCount(m_nRows);
		    pMainFrame->m_GridPM.SetColumnCount(m_nCols);
		    pMainFrame->m_GridPM.SetFixedRowCount(m_nFixRows);
		    pMainFrame->m_GridPM.SetFixedColumnCount(m_nFixCols);
	    }
	    ...
	    for (int row = 0; row < pMainFrame->m_GridPM.GetRowCount(); row++)
        {
	    	for (int col = 0; col < pMainFrame->m_GridPM.GetColumnCount(); col++)
		    { 
                CString str = _T("");	
				GV_ITEM Item;
				Item.mask = GVIF_TEXT;
				...
				Item.strText = str;
				pMainFrame->m_GridPM.SetItem(&Item);
				
				pMainFrame->m_GridPM.SetColumnWidth(col, 80);
				pMainFrame->m_GridPM.SetRowHeight(row, 18);	
	    	}
        }

		//Fill Column Headers
	pMainFrame->m_GridPM.SetItemText(1, 0, _T("R0"));
	pMainFrame->m_GridPM.SetItemText(1, 1, _T("Condition"));	
	pMainFrame->m_GridPM.SetItemText(1, 2, _T("Identifier"));		
	pMainFrame->m_GridPM.SetItemText(1, 3, _T("Attribute"));		
	pMainFrame->m_GridPM.SetItemText(1, 4, _T("Value"));	
	pMainFrame->m_GridPM.SetItemText(1, 5, _T("Operator"));	
	pMainFrame->m_GridPM.SetItemText(1, 6, _T("Production"));
	pMainFrame->m_GridPM.SetItemText(1, 7, _T("Category"));
	pMainFrame->m_GridPM.SetItemText(1, 8, _T("AM"));

	
#if !defined(_WIN32_WCE) || (_WIN32_WCE < 211)
	
		pMainFrame->m_GridPM.AutoSize();
#endif
...
void CAIPITabView::InitGridAgenda(int n_row, int n_col)
{
	...
	pMainFrame->m_GridAgenda.SetItemText(1, 2, _T("Condition"));	
...
int CAIPITabView::FillGridPM() 
{
	...
	pMainFrame->m_wndStatusBar.SetPaneText( 0, _T("Loading production memory(PM) data to grid control..."), RGB(0,0,0), TRUE);
	...
	if( gridMaxRows == 0 )
	{
		pMainFrame->m_wndOutputTabView.AddMsg1(_T("Production memory(PM) status: Empty"));
		InitGridPM(10, gridMaxCols);
		pMainFrame->m_GridPM.ShowWindow(SW_SHOW);
		return 10;
	}
	...
	*/
		...
		TRY 
		{
		    pMainFrame->m_GridPM.SetRowCount(gridMaxRows + 2 );
			pMainFrame->m_GridPM.SetColumnCount(gridMaxCols);
			pMainFrame->m_GridPM.SetFixedRowCount(m_nFixRows);
		    pMainFrame->m_GridPM.SetFixedColumnCount(m_nFixCols);
	...
	//Fill Column Headers
	pMainFrame->m_GridPM.SetItemText(0, 0, _T("C0"));
	pMainFrame->m_GridPM.SetItemText(0, 1, _T("C1"));	
	pMainFrame->m_GridPM.SetItemText(0, 2, _T("C2"));		
	pMainFrame->m_GridPM.SetItemText(0, 3, _T("C3"));		
	pMainFrame->m_GridPM.SetItemText(0, 4, _T("C4"));	
	pMainFrame->m_GridPM.SetItemText(0, 5, _T("C5"));	
	pMainFrame->m_GridPM.SetItemText(0, 6, _T("C6"));
	pMainFrame->m_GridPM.SetItemText(0, 7, _T("C7"));
	pMainFrame->m_GridPM.SetItemText(0, 8, _T("C8"));
	...
	for( iter = pMainFrame->gmLHS.begin(); iter!= pMainFrame->gmLHS.end(); ++iter)
	{
		...
		if( nRow <= gridMaxRows + 1 )
		{
			...
			pMainFrame->m_GridPM.SetItemText(nRow, 0, strRow);
			pMainFrame->m_GridPM.SetItemText(nRow, 1, strCond);
			pMainFrame->m_GridPM.SetItemText(nRow, 2, i.data());		
			pMainFrame->m_GridPM.SetItemText(nRow, 3, a.data());		
			pMainFrame->m_GridPM.SetItemText(nRow, 4, v.data());	
			pMainFrame->m_GridPM.SetItemText(nRow, 5, strRel);	
			pMainFrame->m_GridPM.SetItemText(nRow, 6, strPM);
			pMainFrame->m_GridPM.SetItemText(nRow, 7, strCateg);
		}	
		...
		pMainFrame->m_GridPM.ShowWindow(SW_SHOW);
		str.Format(_T("Production memory(PM) status: %d elements"), gridMaxRows);
		pMainFrame->m_wndOutputTabView.AddMsg1(str);
 			

	m_iMaxRange_PM = pmo.sizePM();
	m_iMinRange_PM = 0;
	


	return m_iMaxRange_PM;
}
...
void CAIPITabView::FillRangeGridPM() 
{
	...
	pMainFrame->m_GridPM.ShowWindow(SW_HIDE);
	pMainFrame->m_GridPM.DeleteAllItems();
	...
	*/
		...
		TRY 
		{
		    pMainFrame->m_GridPM.SetRowCount(2 );
			pMainFrame->m_GridPM.SetColumnCount(gridMaxCols);
		    pMainFrame->m_GridPM.SetFixedRowCount(m_nFixRows);
		    pMainFrame->m_GridPM.SetFixedColumnCount(m_nFixCols);
	...
	for( iter = pMainFrame->gmLHS.begin(); iter!= pMainFrame->gmLHS.end(); ++iter)
	{
		...
		if( pm >= m_iMinRange_PM  && pm <= m_iMaxRange_PM )
		{
			...
			*/
			...
			if( nRow <= gridMaxRows + 1 )
			{
				TRY 
				{
					pMainFrame->m_GridPM.SetRowCount(nRow + 1 );
					pMainFrame->m_GridPM.SetColumnCount(gridMaxCols);
				}
				CATCH (CMemoryException, e)
				{
...
void CAIPITabView::OnGridClick(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
	...
	{
	case GRID_PM:
		ClickGridPM(pItem->iRow, pItem->iColumn);
		break;
	case GRID_WM:
		ClickGridWM(pItem->iRow, pItem->iColumn);
		break;	
	case GRID_AGENDA:
		//str = pMainFrame->m_GridAgenda.GetItemText(pItem->iRow, pItem->iColumn);
		//Trace(str);
		break;
	case GRID_AM:
...
void CAIPITabView::ClickGridPM(int row,int col)
{
	CAipi_PM pmo;
	if( pmo.sizePM() == 0 )
		return;
	...
	bool bEditable = pMainFrame->FindGridVarState(_T("EditablePM"));
	if ( bEditable == true)
		return;
	
	if( col == 1 )
	{
		int nRows = pMainFrame->m_GridPM.GetRowCount();	
		...
		for(int i=0; i<m_vTempRepCond.size(); ++i)
		{
			...
			if( tc + 1 <= nRows )
			{
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 1, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 2, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 3, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 4, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 5, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 6, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 7, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(tc+1, 8, clr1);
			}
		}
		
		pMainFrame->m_GridPM.Invalidate();

		CString strCond = pMainFrame->m_GridPM.GetItemText(row, col);
		...
		{	
			...
			if( c2 + 1 <= nRows )
			{
				...
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 1, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 2, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 3, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 4, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 5, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 6, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 7, clr2);
				pMainFrame->m_GridPM.SetItemBkColour(c2+1, 8, clr2);
	...
	if( col == 6)
	{
		...
		if( !m_vTempPM.empty())
		{
			...
			{
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 1, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 2, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 3, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 4, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 5, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 6, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 7, clr1);
				pMainFrame->m_GridPM.SetItemBkColour(i+1, 8, clr1);
			}
			pMainFrame->m_GridPM.Invalidate();
			m_vTempPM.clear();
		}

		CString strPM = pMainFrame->m_GridPM.GetItemText(row, col);
		...
		for( int i = iniCond; i<= endCond; ++i)
		{
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 1, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 2, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 3, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 4, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 5, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 6, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 7, clr3);
			pMainFrame->m_GridPM.SetItemBkColour(i+1, 8, clr3);
		}
		pMainFrame->m_GridPM.Invalidate();
...
void CAIPITabView::OnBtnRange() 
{
	...
	{
	case GRID_PM:
		m_iMinRange_PM = m_iMinRange;
		m_iMaxRange_PM = m_iMaxRange;
		UpdateData(false);
		FillRangeGridPM();
		break;
	case GRID_WM:
