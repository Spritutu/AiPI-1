...
CAipi_RETE_TK* CAipi_RETE_TK::createTK(int tk, int parent_tk, int wm, int bm, int sign )
{
	/*
	CString str;
...
int CAipi_RETE_TK::findSign(int chld_tk)
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
...
void CAipi_RETE_TK::printTK()
{
	...
	for( CMainFrame::g_mTK::const_iterator iter = pMainFrame->gmTK.begin(); iter!= pMainFrame->gmTK.end(); ++iter)
	{
		...
		int sign = tko.getSign();
		strSign +=  _itot( sign, buffer, 10 );
		pMainFrame->m_wndOutputTabView.AddMsg1(strSign);
